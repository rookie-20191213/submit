...
if __name__=='__main__':
    ...
    data=np.load('OverThrust.npy')
    data=skimage.transform.resize(data,(200,400))
    data[:5,:]=1
    true_model=data[:,select_index]
    pyplot.figure(figsize=(8,6))
    pyplot.rcParams.update({'font.size': 15})
    gci=pyplot.imshow(data,extent=(0,400,200,0),cmap=cm.gray_r,vmin=1,vmax=8)
    ax=pyplot.gca()
    ...
    data=create_model.Initial_Smooth_Model(iepsilon_,30,0)
    data[:5,:]=1
    ...
    freq=2e8
    dir_path='./Ori/%sHz_imodel_file'%freq
    file_num=int(len(list(Path(dir_path).iterdir())))
    data=np.load('%s/%s_imodel.npy'%(dir_path,file_num-1))

    data=data.reshape((220,-1))
    data=data[10:-10,10:-10]
    pyplot.figure(figsize=(8,6))
    pyplot.rcParams.update({'font.size': 15})
    gci=pyplot.imshow(data,extent=(0,400,200,0),cmap=cm.gray_r)
    ax=pyplot.gca()
    ax.set_xticks(np.linspace(0,400,5))
    ax.set_xticklabels([0,2,4,6,8])
    ax.set_yticks(np.linspace(0,200,5))
    ...
    # cbar.formatter.set_powerlimits((-1, 1))
    cbar.set_label('$\epsilon_r$')
    pyplot.savefig('02_without_regularization_2e8.png',dpi=1000)
    ...
    pyplot.rcParams.update({'font.size': 15})
    l0=pyplot.plot(true_model,'k--',label='True Model')
