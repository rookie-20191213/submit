...
def find_next_zero_crossing(signal, start_idx=0):
    """
    Find the next index i >= start_idx where the signal crosses or hits zero.
    A 'crossing' is where consecutive samples differ in sign.
    ...
    for i in range(start_idx, n-1):
        # Exactly zero or sign change
        if signal[i] == 0 or np.sign(signal[i]) != np.sign(signal[i+1]):
            return i
    return None

def cut_after_zero(signal, threshold):
    """
    1. Find the first wave start where signal > threshold.
    2. Find where it drops below threshold again.
    3. From that index, find the next zero crossing.
    4. Zero out everything after that crossing to ensure the wave is fully included.

    Args:
        signal    : 1D numpy array
        threshold : amplitude threshold that defines "start" and "end" of wave

    Returns:
        modified_signal : the result with post-wave region set to zero
        i_start         : index of wave start (signal first > threshold)
        i_thresh_end    : index where signal returns below threshold
        i_zero_end      : index of the next zero crossing after i_thresh_end
    """
    sig_copy = signal.copy()
    n = len(signal)

    # 1) Wave start
    candidates_start = np.where(sig_copy > threshold)[0]
    if len(candidates_start) == 0:
        print("No wave start found above threshold.")
        return sig_copy, None, None, None
    i_start = candidates_start[0]

    # 2) Wave threshold end
    #    from i_start onward, find the first index where signal < threshold
    candidates_end = np.where(sig_copy[i_start:] < threshold)[0]
    if len(candidates_end) == 0:
        print("Wave never went back below threshold.")
        return sig_copy, i_start, None, None
    i_thresh_end = i_start + candidates_end[0]

    # 3) Next zero crossing after i_thresh_end
    i_zero_end = find_next_zero_crossing(sig_copy, start_idx=i_thresh_end)
    if i_zero_end is None:
        print("No zero crossing found after wave ended at threshold.")
        return sig_copy, i_start, i_thresh_end, None

    # 4) Zero out everything after i_zero_end
    sig_copy[i_zero_end+1:] = 0
...
if __name__=='__main__':
    s_time=time.time()
    X_data=np.load('forward_without_air.npy')
    max_iter=1000
    rank=1
    lam=1e-4
    method='GoDec'
    ...
    # np.save('ref_data.npy',RefData)
    # pyplot.figure()
    # pyplot.imshow(X_data-RefData,extent=(0,1,0,1),vmin=-10,vmax=10)
    # np.save('without_ref_data.npy',X_data-RefData)
